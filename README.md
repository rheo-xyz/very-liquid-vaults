...
10. The system assumes that integrated strategies are honest and non-malicious. A malicious or gas-griefing strategy could revert or consume excessive gas in `totalAssets` or other operations. Additionally, if a legitimate integrated protocol (e.g., Aave, Morpho) temporarily pauses or experiences an oracle outage, its strategy's `convertToAssets` call may revert, causing `totalAssets()` to revert. Since `totalAssets()` is called by the `mintPerformanceFee` modifier, this cascades into a temporary denial of service on all vault operations (`deposit`, `mint`, `withdraw`, `redeem`) until the external protocol resumes or the guardian removes the affected strategy via `removeStrategy`. This is a deliberate fail-safe design: silently treating a reverting strategy's assets as zero would cause incorrect share pricing and potential loss of funds for remaining depositors. The `totalAssets()` function does not use `try/catch` by design, as underreporting total assets is considered a worse outcome than a temporary pause. The guardian role (`GUARDIAN_ROLE`) can mitigate this by calling `removeStrategy` with `amount = 0` to remove the problematic strategy without attempting to rebalance its assets.
...
13. `VeryLiquidVault` can violate ERC-4626's preview guarantees for `deposit` and `mint` when its own share price diverges from the minimum deposit granularity of the strategy that actually receives the final routed assets. In particular, `previewDeposit(assets)` may return a positive value, and `assets` may still be `<= maxDeposit()`, yet `deposit(assets, receiver)` can revert if the routing loop leaves a remainder that is too small to mint shares in the next strategy. This can happen once a yield-bearing strategy such as Aave has accrued any yield, because its minimum depositable amount becomes greater than 1 unit while `VeryLiquidVault` may still preview the same amount at roughly 1:1 using its aggregate exchange rate. The same issue applies to `mint`, since OpenZeppelin ERC-4626 computes `assets = previewMint(shares)` and then executes the same `_deposit` routing logic. Operationally, this means small "dust" deposits can revert, larger deposits can also revert when they leave a dust remainder after filling earlier strategy capacity, `max{Deposit,Mint}` can be overstated for those boundary amounts, and an unprivileged actor can grief deposits by temporarily filling earlier strategy capacity so a victim's transaction is forced to send an unfillable remainder into the next strategy.
14. `MorphoVaultV2StrategyVault`'s liquidity resolver assumes the underlying Morpho Vault V2's active `liquidityAdapter` is a Morpho Market V1 adapter. If a different adapter type is configured, or any of the underlying reads fail or `liquidityData` does not decode to a Morpho Blue `MarketParams`, the resolver conservatively degrades its `maxWithdraw` to idle-only liquidity (the V2 vault's unallocated asset balance). This may understate the truly exitable amount but never overstates it. Separately, `maxDeposit` is gate-only: it honors the strategy and `VeryLiquidVault` `totalAssetsCap` and the V2 entry gate, but does not subtract the liquidity adapter's market cap headroom, so deposit capacity can be overstated. Deposit overstatement is cheaply contained by `VeryLiquidVault`'s per-strategy `try/catch` on deposit and by off-chain rebalance sizing.
15. A vault with a nonzero `performanceFeePercent` crystallizes its pending fee through the `mintPerformanceFee` modifier, i.e. only inside `deposit`/`mint`/`withdraw`/`redeem`, and never inside a view. Consequently, while a fee is pending (current PPS above the high-water mark), `previewRedeem`, `previewWithdraw`, `maxWithdraw`, and `maxRedeem` are computed from the pre-mint supply and are optimistic relative to the same-transaction state-changing call: the subsequent `redeem` returns fewer assets and the subsequent `withdraw` burns more shares than previewed, deviating from ERC-4626's preview bounds. Entry previews (`previewDeposit`/`previewMint`) are unaffected in the conservative direction, since the fee mint increases supply before entry pricing. **This becomes value-transferring when a fee-bearing `VeryLiquidVault` is used as a strategy of another `VeryLiquidVault`**, a supported configuration: the outer vault values that position with `strategy.convertToAssets(strategyBalance)` in `totalAssets()`, which is the gross, pre-dilution claim, and OpenZeppelin's ERC-4626 fixes the outer redemption's asset amount *before* calling `VeryLiquidVault._withdraw`. The inner fee is only minted when `_withdraw` reaches `strategy.withdraw(...)`, by which point the outer vault must deliver an amount it already priced at gross NAV, so it burns additional inner shares to cover it. With `G` the outer vault's gross claim on the inner vault, `F` the pending inner fee, `N = G - F`, and `f` the redeemer's fraction of outer shares, the exit succeeds whenever `f * G <= N` and pays `f * G` instead of the fee-first `f * N`; the redeemer gains `f * F` and the remaining outer LPs absorb it. Because the success bound `f <= 1 - F/G` is looser than the break-even bound `f <= 1 - q` (for fee rate `q`, whenever principal is positive), a sufficiently large first exit can shift more than the remaining LPs' unclaimed yield and eat into their principal. This is a first-mover race, permissionless and deterministic, requiring no role compromise, oracle manipulation, reentrancy, or malicious strategy. Operationally, **nested `VeryLiquidVault` strategies must keep `performanceFeePercent` at zero**; charge the performance fee only at the outermost vault. No currently deployed vault combines nesting with a nonzero inner performance fee.
